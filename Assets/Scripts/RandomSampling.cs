...
public class RandomSampling
{
    ...
    public static List<Point> GeneratePoints(int seed, float maxRadius, float minRadius, float sampleRegionSize, int iterationsPerCell)
    {
        ...
        for (int i = 0; i <= gridSize; i++)
            for (int j = 0; j <= gridSize; j++)
            {
                ...
                for (int k = 0; k < iterationsPerCell; k++)
                {
                    if (invalid)
                    {
                        tempPoint = new Point(gridX + Random.Range(0f, maxRadius), gridY + Random.Range(0f, maxRadius), Random.Range(minRadius, maxRadius));
                        invalid = false;
                    }
